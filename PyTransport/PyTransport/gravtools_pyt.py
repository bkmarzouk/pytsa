...
class Curvature:
	...
	def __init__(self, metric, coords, params=None, precompute=False):
		...
		phidot_syms_d = [sum(self.metric[i, j] * phidot_syms_u[i] for j in range(nF)) for i in range(nF)]
		phidotdot_syms_d = [sum(self.metric[i, j] * phidotdot_syms_u[i] for j in range(nF)) for i in range(nF)]
		...
		KE_sym = sum(
			[sym.Rational(1, 2) * self.metric[i, j] * phidot_syms_u[i] * phidot_syms_u[j] for i in range(nF) for j in
			 range(nF)])

		# Decompose matrix into 3 terms
		Hij = np.zeros((nF, nF), dtype=object)  # Covariant Hessian
		Cij = np.zeros((nF, nF), dtype=object)  # Riemann Geometric term
		Kij = np.zeros((nF, nF), dtype=object)  # Kinetic term

		rnf = range(nF)

		for i in rnf:
			for j in rnf:
				Hij[i, j] = ddV[i, j] - sum(self.Csyms[k, i, j] * dV[k] for k in rnf)

				Cij[i, j] = sum(-self.Rsyms[i, k, l, j] * phidotdot_syms_u[k] * phidotdot_syms_u[l] for k in rnf for l in rnf)

				Kij_1 = -(3 + KE_sym / H_sym / H_sym) * phidot_syms_d[i] * phidot_syms_d[j]

				Kij_2 = -(phidot_syms_d[i] * phidotdot_syms_d[j] + phidotdot_syms_d[i] * phidot_syms_d[j]) / H_sym

				Kij_3 = sum(
					phidot_syms_u[k] * (
						self.Csyms[m, k, i] * phidot_syms_d[j] + self.Csyms[m, k, j] * phidot_syms_d[i]
					) * phidot_syms_d[m] / H_sym for k in rnf for m in rnf
				)

				Kij[i, j] = Kij_1 + Kij_2 + Kij_3

		self.Hij = Hij  # Covariant Hessian term
		self.Cij = Cij  # Riemann term
		self.Kij = Kij  # Kinetic (covariant time derivative) term
