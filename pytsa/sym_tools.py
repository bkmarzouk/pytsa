...
def riemann_operation_matrix(N, verbose=True):
    """
    Computes matrix of operations for independent components of
    the Riemann tensor
    ...
    for ii in range(N):
        ...
        for jj in jrange:
            ...
            if ii < jj:
                ...
                # Prescribe dependent components in terms of fundamental
                # component * +/-
                subs = [ii, jj, ii, jj]
                pijij = "+{}{}{}{}".format(*subs)
                mijij = "-{}{}{}{}".format(*subs)
                ...
                OM[jj, ii, ii, jj] = mijij
                OM[ii, jj, jj, ii] = mijij

                # Skewing both pairs of terms leaves the sign unchanged
                OM[jj, ii, jj, ii] = pijij

                # Since symmetric over index pairs,
                # exchange symmetry need not be considered
            ...
            for kk in krange:
                ...
                if kk < jj:
                    ...
                    OM[jj, ii, kk, ii] = mijki
                    OM[ii, jj, ii, kk] = mijki
                    OM[jj, ii, ii, kk] = pijki

                    # Since first and last pairs carry different index pairs,
                    # consider exchange symmetries
                    OM[kk, ii, ii, jj] = pijki
                    OM[kk, ii, jj, ii] = mijki
                    OM[ii, kk, ii, jj] = mijki
                    OM[ii, kk, jj, ii] = pijki
                ...
                for ll in lrange:
                    # Choose irreducible representation to be those with
                    # i < j & k < l in first & 2nd index pairs.
                    # To avoid double counting under exchange symmetry,
                    # we items with lowest index in first index pair
                    if ii < jj and kk < ll and min(ii, jj) <= min(kk, ll):
                        OM[ii, jj, kk, ll] = "calc"
                        idp_ijkl += 1

                        # Prescribe dependent components in terms of
                        ...
                        # Skewing first and last pair incurs a minus sign
                        OM[jj, ii, kk, ll] = mijkl
                        OM[ii, jj, ll, kk] = mijkl
                        OM[jj, ii, ll, kk] = pijkl

                        # Since first and last pairs carry different
                        # index paris, consider exchange symmetry
                        OM[kk, ll, ii, jj] = pijkl
                        OM[kk, ll, jj, ii] = mijkl
                        OM[ll, kk, ii, jj] = mijkl
                        OM[ll, kk, jj, ii] = pijkl

                        bterms.append([ii, jj, kk, ll])
    ...
    def exch(ii, jj, kk, ll):
        return [kk, ll, ii, jj]

    for indices in bterms_copy:
        ii, jj, kk, ll = indices
        ...
        # We have then the following possible representations for
        # the Riemann tensor
        con1 = (b1 in bterms or exch(*b1) in bterms) and (
            b2 in bterms or exch(*b2) in bterms
        )
        con2 = (b1s in bterms or exch(*b1s) in bterms) and (
            b2s in bterms or exch(*b2s) in bterms
        )
        con3 = (b1 in bterms or exch(*b1) in bterms) and (
            b2s in bterms or exch(*b2s) in bterms
        )
        con4 = (b1s in bterms or exch(*b1s) in bterms) and (
            b2 in bterms or exch(*b2) in bterms
        )

        if con1:
            bterms.remove(indices)
            idp_ijkl += -1
            OM[ii, jj, kk, ll] = "-{}{}{}{}".format(*b1) + "-{}{}{}{}".format(
                *b2
            )
            ...
            bterms.remove(indices)
            idp_ijkl += -1
            OM[ii, jj, kk, ll] = "+{}{}{}{}".format(*b1s) + "+{}{}{}{}".format(
                *b2s
            )
    ...
    assert 2 * idp_ijij == N * (N - 1), (
        str(idp_ijij) + "/" + str(N * (N - 1) / 2)
    )
    assert 2 * idp_ijki == N * (N - 1) * (N - 2), (
        str(idp_ijki) + "/" + str(N * (N - 1) * (N - 2) / 2)
    )
    assert 12 * idp_ijkl == N * (N - 1) * (N - 2) * (N - 3), (
        str(idp_ijkl) + "/" + str(N * (N - 1) * (N - 2) * (N - 3) / 12)
    )

    if verbose:
        print(
            "-- Independent components: {}".format(
                idp_ijij + idp_ijki + idp_ijkl
            )
        )
...
def FieldSpaceSym(
    ...
    try:
        fs_derived = cache_tools.load_fmet(
            metric, delete=recache, simplify=simplify
        )
...
def PotentialSym(
    ...
    try:
        pot_derived = cache_tools.load_pot(
            potential, delete=recache, simplify=simplify
        )
...
def CovDSym(
    ...
    try:
        covd_derived = cache_tools.load_covd(
            metric,
            potential,
            delete=recache,
        ...
        covd_derived = _CovariantDerivativesSym(
            n_fields,
            n_params,
            metric,
            potential,
...
class _FieldSpaceSym(object):
    ...
    def __init__(
        ...
        self.p_syms = (
            sym.symarray("f", n_params) if n_params > 0 else None
        )  # param symbols
    ...
    def _configure_metric(self, metric):
        ...
        assert (
            self.metric.shape[0] == self.nf
        ), "Metric dimensions do not match number of fields: {}".format(
            self.nf
        )
        ...
        for a in self.nf_range:
            for b in self.nf_range:
                if b >= a:
                    self.metric_inverse[a, b] = sym.simplify(
                        self.metric_inverse[a, b]
                    )
                else:
                    self.metric_inverse[b, a] = self.metric_inverse[
                        a, b
                    ]  # symmetry
        ...
        for ii in range(2 * self.nf * 2 * self.nf):
            sympy_matrix_flattened[ii] = sym.simplify(
                combined_matrix_flattened[ii]
            )
    ...
    def _compute_christoffel(self, a, b, c):
        ...
        if not self.canonical:
            ...
            for d_idx, d in enumerate(self.f_syms):
                christoffel += (
                    sym.Rational(1, 2)
                    * self.metric_inverse[a_idx, d_idx]
                    * sum(
                        [
                            sym.diff(
                                self.metric[d_idx, c_idx], b
                            ),  # partial_b g_{dc} +
                            sym.diff(
                                self.metric[b_idx, d_idx], c
                            ),  # partial_c g_{bd} -
                            -sym.diff(
                                self.metric[b_idx, c_idx], d
                            ),  # partial_d g_{bc}
                        ]
                    )
                )
    ...
    def _configure_christoffel_symbols(self):
        ...
        self.christoffel_symbols = np.zeros(
            (self.nf, self.nf, self.nf), dtype=object
        )
                    ...
                    for c in self.nf_range:
                        if c >= b:  # symmetry on lower indices
                            self.christoffel_symbols[
                                a, b, c
                            ] = self._compute_christoffel(a, b, c)
                        else:
                            self.christoffel_symbols[
                                a, b, c
                            ] = self.christoffel_symbols[a, c, b]

    def _compute_mixed_riemann_tensor(self, f, c, a, b):
        """
        Computes element of Riemann tensor with first index raised
        and rest lower
        ...
        if not self.canonical:
            ...
            term3 = sum(
                [
                    self.christoffel_symbols[f_idx, a_idx, d_idx]
                    * self.christoffel_symbols[d_idx, b_idx, c_idx]
                    - self.christoffel_symbols[f_idx, b_idx, d_idx]
                    * self.christoffel_symbols[d_idx, a_idx, c_idx]
    ...
    def _compute_lowered_riemann_tensor(self, d, c, a, b):
        ...
        if not self.canonical:
            ...
            Rdcab = sum(
                [
                    self.metric[d_idx, f_idx]
                    * self._compute_mixed_riemann_tensor(
                        f_idx, c_idx, a_idx, b_idx
                    )
                    for f_idx in self.nf_range
                ]
            )
    ...
    def _configure_riemann_tensor(self):
        ...
        self.riemann_tensor = np.zeros(
            (self.nf, self.nf, self.nf, self.nf), dtype=object
        )

        om = riemann_operation_matrix(self.nf)

        assert (
            self.riemann_tensor.shape == om.shape
        ), "Operation matrix incompatible shape with Riemann tensor"
        ...
        for ijkl in calc_ijkl:
            ...
            assert (
                self.riemann_tensor[ii, jj, kk, ll] == 0
            ), "Riemann component already has assigned value! {}".format(
                self.riemann_tensor[ii, jj, kk, ll]
            )

            Rijkl = self._compute_lowered_riemann_tensor(ii, jj, kk, ll)

            self.riemann_tensor[ii, jj, kk, ll] = Rijkl
        ...
        for ijkl in symm_ijkl:
            ...
            # Unpack the string that points to the fundamental Matrix
            # element(s) and multiplicative sign
            str_pointer = om[ii, jj, kk, ll]
            ...
            if len(str_pointer) == 5:
                ...
                if sign == "+":
                    self.riemann_tensor[ii, jj, kk, ll] += Rpqrs
                elif sign == "-":
                    self.riemann_tensor[ii, jj, kk, ll] += -Rpqrs
                else:
                    raise ValueError(
                        "Unrecognized sign value: {}".format(sign)
                    )
                ...
                if sign1 == "+":
                    ...
                    raise ValueError(
                        "Unrecognized sign value: {}".format(sign1)
                    )

                if sign2 == "+":
                    self.riemann_tensor[ii, jj, kk, ll] += Rtuvw
                elif sign2 == "-":
                    self.riemann_tensor[ii, jj, kk, ll] += -Rtuvw
                else:
                    raise ValueError(
                        "Unrecognized sign value: {}".format(sign2)
                    )

            else:
                raise ValueError(
                    "Unrecognized symmetry relation: {}".format(str_pointer)
                )


class _PotentialSym(object):
    def __init__(
        self, n_fields: int, n_params: int, potential: sym.Expr, simplify: bool
    ):
        self.nf = n_fields
        self.nf_range = list(range(n_fields))
        self.f_syms = sym.symarray("f", n_fields)  # field symbols
            ...
            for jj, f_jj in enumerate(self.f_syms):
                ...
                for kk, f_kk in enumerate(self.f_syms):
                    if kk >= jj:
                        ...
                        self.pdpdpd_potential[
                            ii, jj, kk
                        ] = self.pdpdpd_potential[ii, kk, jj]
...
class _CovariantDerivativesSym(object):
    def __init__(
        ...
        self.fmet_sym = FieldSpaceSym(
            n_fields, n_params, metric, simplify=simple_metric
        )
        self.pot_sym = PotentialSym(
            n_fields, n_params, potential, simplify=simple_potential
        )
        ...
        cache_tools.cache_covd(
            metric,
            potential,
            self,
            simplify_fmet=simple_metric,
            simplify_pot=simple_potential,
    ...
    def _compute_covd_riemann(self, e, d, c, a, b):
        """Compute the covariant derivative of the
        Riemann tensor \nabla_e R_{dcab}"""
    ...
    def _configure_covd_riemann(self):
        ...
        self.covd_riemann = np.zeros(
            (self.nf, self.nf, self.nf, self.nf, self.nf), dtype=object
        )

        # can borrow symmetries from riemann tensor to exploit syms
        OM = riemann_operation_matrix(self.nf, verbose=False)

        # Get location of independent elements to compute & symmetrically
        # related components
        calc_locs = np.where(OM == "calc")
        symm_locs = np.where(np.logical_and(OM != "calc", OM != 0))
        ...
        for deriv_idx in self.nf_range:
            ...
            for ijkl in calc_ijkl:
                ...
                covd_riemann = self._compute_covd_riemann(
                    deriv_idx, ii, jj, kk, ll
                )

                if self.simplify:
                    covd_riemann = sym.simplify(covd_riemann)

                self.covd_riemann[deriv_idx, ii, jj, kk, ll] = covd_riemann

            for ijkl in symm_ijkl:
                ii, jj, kk, ll = ijkl

                assert self.covd_riemann[deriv_idx, ii, jj, kk, ll] == 0, (
                    "Cov D Riemann component already has assigned value! "
                    "{}".format(self.covd_riemann[deriv_idx, ii, jj, kk, ll])
                )
                ...
                if len(str_pointer) == 5:
                    ...
                    covdRpqrs = self.covd_riemann[
                        deriv_idx, int(p), int(q), int(r), int(s)
                    ]
                    ...
                    covdRtuvw = self.covd_riemann[
                        deriv_idx, int(t), int(u), int(v), int(w)
                    ]
                    ...
                    raise ValueError(
                        "Unrecognized symmetry relation: {}".format(
                            str_pointer
                        )
                    )
    ...
    def _compute_covd_potential(self, a):
        ...
        covd_potential = self._get_pd_potential(
            a_idx
        )  # simply same as partial derivative at 1st derivative
    ...
    def _compute_covdcovd_potential(self, a, b):
        ...
        out = self._get_pdpd_potential(
            a, b
        )  # canonical fmet -> second partial derivative

        if not self.fmet_sym.canonical:  # NC has christoffel term
            connexion_term = -sum(
                [
                    self._get_christoffel_symbol(k_idx, a_idx, b_idx)
                    * self.covd_potential[k_idx]
    ...
    def _configure_covdcovd_potential(self):
        ...
        for a in self.nf_range:
            for b in self.nf_range:
                if a >= b:
                    self.covdcovd_potential[
                        a, b
                    ] = self._compute_covdcovd_potential(a, b)
                else:
                    self.covdcovd_potential[a, b] = self.covdcovd_potential[
                        b, a
                    ]
    ...
    def _compute_covdcovdcovd_potential(self, a, b, c):
        ...
        out = self._get_pdpdpd_potential(
            a, b, c
        )  # canonical is simply thrid partial derivative

        if not self.fmet_sym.canonical:
            t1 = -sum(
                [
                    sym.diff(
                        self._get_christoffel_symbol(k_idx, b_idx, c_idx)
                        * self._get_pd_potential(k_idx),
                        a,
                    )
                    for k_idx in self.nf_range
                ]
            ...
            t2 = -sum(
                [
                    self._get_christoffel_symbol(d_idx, a_idx, b_idx)
                    * self.covdcovd_potential[d_idx, c_idx]
                    + self._get_christoffel_symbol(d_idx, a_idx, c_idx)
                    * self.covdcovd_potential[d_idx, b_idx]
                    for d_idx in self.nf_range
                ]
            )
    ...
    def _configure_covdcovdcovd_potential(self):
        ...
        self.covdcovdcovd_potential = np.zeros(
            (self.nf, self.nf, self.nf), dtype=object
        )
        ...
        for a in self.nf_range:
            for b in self.nf_range:
                for c in self.nf_range:
                    if c >= b:
                        self.covdcovdcovd_potential[
                            a, b, c
                        ] = self._compute_covdcovdcovd_potential(a, b, c)
                    else:
                        self.covdcovdcovd_potential[
                            a, b, c
                        ] = self._compute_covdcovdcovd_potential(a, c, b)
    ...
    def get_curvature_sym_arrays(self):
        ...
        for ii in range(self.nf):
            for jj in range(self.nf):
                for kk in range(self.nf):
                    for ll in range(self.nf):
                        riemann[
                            self.nf**3 * ii
                            + self.nf**2 * jj
                            + self.nf * kk
...
if __name__ == "__main__":
    ...
    V = (
        sym.Rational(1, 2) * p[0] ** 2.0 * f[0] ** 2
        + sym.Rational(1, 2) * p[1] ** 2 * f[1] ** 2
    )
    ...
    for ii in range(n):
        for jj in range(n):
            for kk in range(n):
                for ll in range(n):
                    if len(list(set([ii, jj, kk, ll]))) == 4:
                        print([ii, jj, kk, ll], OM[ii, jj, kk, ll])
