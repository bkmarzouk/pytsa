...
from .setup_sampler import APrioriSampler, LatinSampler
...
class SampleData:
    def __init__(
        self, n_samples: int, index: int, task_dict: dict or None = None
    ):
...
def write_results(args_dict: dict, indices: np.ndarray):
    ...
    samples_dir = os.path.join(
        args_dict["cwd"], args_dict["name"], "samples_core"
    )
    ...
    results_path = os.path.join(
        sampler_run_dir, "pandas_{}.df".format(args_dict["name"])
    )
    ...
    # Filter any rows that contain bad data values that would corrupt
    # getdist analysis
    ...
    getdist_p_path = os.path.join(
        sampler_run_dir, f"getdist_{name}.paramnames"
    )
