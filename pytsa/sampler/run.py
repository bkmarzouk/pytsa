...
if __name__ == "__main__":
    ...
    import schwimmbad
    from pytsa.sampler.mpi_helpers import mpi_size as n_proc

    # Import argument parser for cmd args
    from argparse import ArgumentParser

    parser = ArgumentParser(
        description="Configure PyTransport sampler Routine"
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        dest="verbose",
        help="Run sampler in verbose mode",
    )
    ...
    sampler_group.add_argument(
        ...
        action="store_true",
        dest="task_2pt",
        help="If flagged, computes 2-point data",
    )
    sampler_group.add_argument(
        "--eq",
        action="store_true",
        dest="task_3pt_eq",
        help="If flagged, computes 3-point data in equilateral mode "
        "configuration",
    )
    sampler_group.add_argument(
        "--fo",
    ...
    sampler_group.add_argument(
        ...
        action="store_true",
        dest="task_3pt_sq",
        help="If flagged, computes 3-point data in squeezed mode "
        "configutarion",
    )
    ...
    extra_group.add_argument(
        ...
        nargs="+",
        required=False,
        help="Pass values of alpha (Fergusson Shellard convention) to "
        "compute custom fnls. "
    ...
    extra_group.add_argument(
        ...
        nargs="+",
        required=False,
        help="Pass values of beta (Fergusson Shellard convention) "
        "to compute custom fnls. "
    ...
    assert len(parsed_args.alpha) == len(parsed_args.beta), [
        parsed_args.alpha,
        parsed_args.beta,
    ]
    ...
    os.environ["pytsa_SAMPLER"] = os.path.join(
        args_dict["cwd"], args_dict["name"], "sampler.run"
    )

    pool = schwimmbad.choose_pool(
        mpi=n_proc > 1, processes=n_proc, use_dill=False
    )
